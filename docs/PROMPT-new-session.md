...
whose cwd is the dotskills repository root.
...
Work only in the dotskills repository. The one exception is creating/replacing the symlink at the parent directory's `setup-ai-tools.sh` → this repo's `bin/setup-ai-tools.sh`.
...
4. Refresh `skills/setup-rs-guard/SKILL.md` to rs-guard **v1.6.0** and **`deepseek-v4-pro`** so the opt-in extra is current. Verify facts against the rs-guard repository (`docs/CONFIGURATION.md`, `docs/USAGE.md`, current schema). Replace any stale machine-specific paths. Do not invent config keys. Keep the file in the tree; do not install it on a default `./install.sh`.
