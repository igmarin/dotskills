...
4. Refresh `skills/setup-rs-guard/SKILL.md` to rs-guard **v1.7.0** and **`deepseek-v4-pro`** so the opt-in extra is current. Verify facts against the rs-guard repository (`docs/CONFIGURATION.md`, `docs/USAGE.md`, current schema). Replace any stale machine-specific paths. Do not invent config keys. Keep the file in the tree; do not install it on a default `./install.sh`.
