...
7. Behavior that must not change:
...
Move the `process_project` CodeGraph block into `bin/lib/codegraph.sh` (one function, e.g. `codegraph_setup "$repo"`). Keep the three-way rule in one place.

**Done when:** `.codegraph` present → no “Initializing” / no `init`. `--force` with index present → `sync` only.

### 3. Extract Graphify

Move `ensure_graphify_mcp_extra`, `ensure_graphify_ignore`, and the extract block into `bin/lib/graphify.sh`. Extra stays `graphifyy[mcp,openai]`.
...
Move global gitignore, Devin / Cline / Grok / Zed global setup, `upsert_grok_mcp_toml`, and the per-repo JSON/TOML/clinerules writers into `bin/lib/write-mcp.sh`.
